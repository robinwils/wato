...
#define CHECK_VEC3_APPROX_VALUES(v1, xx, yy, zz) \
    do {                                         \
        CHECK_EQ(v1.x, doctest::Approx(xx));     \
        CHECK_EQ(v1.y, doctest::Approx(yy));     \
        CHECK_EQ(v1.z, doctest::Approx(zz));     \
    } while (0)
...
#define CHECK_VEC3_APPROX_EPSILON_VALUES(v1, xx, yy, zz, eps) \
    do {                                                      \
        CHECK_EQ(v1.x, doctest::Approx(xx).epsilon(eps));     \
        CHECK_EQ(v1.y, doctest::Approx(yy).epsilon(eps));     \
        CHECK_EQ(v1.z, doctest::Approx(zz).epsilon(eps));     \
    } while (0)
